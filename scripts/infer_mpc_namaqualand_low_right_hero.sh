...
# 오른쪽 아래에서 올려다봄: fx=-0.5 (오른쪽에서), fy=+0.7 (정면), fz=+0.4 (아래에서 올려다봄)
# uy=+0.4 (카메라가 위를 향함)
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" python scripts/infer_mpc_blender.py \
  ...
  --disable_roll \
  --target_json '{"center_x":0.5,"center_y":0.5,"occupancy":0.4,"aspect_ratio":1.0,"camera_to_object_fx":-0.5,"camera_to_object_fy":0.7,"camera_to_object_fz":0.4,"camera_to_object_ux":-0.3,"camera_to_object_uy":0.4,"camera_to_object_uz":0.8}'
