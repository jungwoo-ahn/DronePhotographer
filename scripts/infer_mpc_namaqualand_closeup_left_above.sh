...
# 중앙 클로즈업: 50% occupancy, 오른쪽 위에서 정면 (fx=-0.5=오른쪽에서, fy=+0.7=정면, fz=-0.3=위에서)
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" python scripts/infer_mpc_blender.py \
  ...
  --disable_roll \
  --target_json '{"center_x":0.5,"center_y":0.5,"occupancy":0.5,"aspect_ratio":0.8,"camera_to_object_fx":-0.5,"camera_to_object_fy":0.7,"camera_to_object_fz":-0.3,"camera_to_object_ux":0.4,"camera_to_object_uy":-0.4,"camera_to_object_uz":0.8}'
