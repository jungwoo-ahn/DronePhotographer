"""Evaluate the π0-style VLA baseline on a target shot profile.

Mirrors `scripts/eval_cosmos_policy.py` so the two are directly comparable:
same v7 start frame, same goal YAML, same pose-proxy distance metric and JSON
output — only the model differs (Qwen3-VL + flow action expert, no world model).

  python scripts/eval_vla_policy.py \
      --checkpoint runs/<ts>_vla_qwen3_2b/ckpt_last.pt \
      --start_annotation data/trajectories_full/<placement>/data.json \
      --target configs/policy/targets/centered_medium.yaml [--n_steps 10]
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--repo_id", default="Qwen/Qwen3-VL-2B-Instruct")
    p.add_argument("--resolution", nargs=2, type=int, default=[480, 720])
    p.add_argument("--dtype", default="bfloat16")
    p.add_argument("--device", default="cuda")
    p.add_argument("--n_steps", type=int, default=10, help="flow integration steps")
...
def main() -> None:
    ...
    backbone = Qwen3VLModel.from_pretrained(args.repo_id, torch_dtype=dtype)
    ...
    with torch.no_grad(), torch.amp.autocast(device.type, dtype=dtype):
        vlm_inputs, goal, _ = policy.prepare_inputs(batch, device, dtype)
        out = policy.sample(vlm_inputs, goal, n_steps=args.n_steps)   # denormalized (m/rad)
