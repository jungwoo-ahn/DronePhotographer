...
# 왼쪽 프로필 (fx=+0.7) + 약간 정면 (fy=+0.5), 눈높이, 포트레이트 비율
# center_x=0.45 — 시선 방향 쪽에 살짝 여백
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" python scripts/infer_mpc_blender.py \
  ...
  --disable_roll \
  --target_json '{"center_x":0.45,"center_y":0.5,"occupancy":0.45,"aspect_ratio":0.75,"camera_to_object_fx":0.7,"camera_to_object_fy":0.5,"camera_to_object_fz":0.0,"camera_to_object_ux":0.0,"camera_to_object_uy":0.0,"camera_to_object_uz":1.0}'
