...
from ..util.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       accuracy_best,accuracy_best_10, get_world_size, interpolate,
                       is_dist_avail_and_initialized, inverse_sigmoid)

from .backbone import build_backbone
# VENDORED FOR INFERENCE (DronePhotographer UNIC baseline): the training-only
# matcher and the segmentation module (which pulls panopticapi) are not vendored,
# and cv2 (used only by training-time visualization) is made optional. These names
# are referenced only by the training `build()` / `SetCriterion` paths, which the
# baseline never calls — we construct the model + PostProcess directly. See
# src/policy/unic/REFERENCES.md.
...
class ConditionalDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, part:Boolean,label_class):
        ...
        hs, reference,memory = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1],part,label_class)
        ...
        if part==False:
            ...
            if label_class=='soft':
                ...
                out = {'pred_logits': outputs_class[-1].detach(), 'pred_boxes': outputs_coord[-1].detach(),'memory':memory}
                if self.aux_loss:
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets_all,outputs_ema, indices, num_boxes, log=True,label_class='soft'):
        ...
        if label_class=='SD':
            # print("allmetema")
            src_logits_ema = outputs_ema['pred_logits']
            idx = self._get_src_permutation_idx(indices)
            ...
            loss_ce_ema = sigmoid_focal_loss(src_logits, src_logits_ema.sigmoid(), num_boxes, alpha=self.focal_alpha, gamma=2) * \
                    src_logits.shape[1]
            loss_ce = loss_ce +  loss_ce_ema

            losses = {'loss_ce': loss_ce}
           
        if label_class=='onlySD':
            # print("allmetema")
            src_logits_ema = outputs_ema['pred_logits']
            idx = self._get_src_permutation_idx(indices)
        ...
        if log:
            ...
            losses['class_error_75'] =  accuracy_best_10(outputs, targets_all,iou_threshold=0.90)[0]
            losses['class_error_70'] =  accuracy_best_10(outputs, targets_all,iou_threshold=0.85)[0]
        return losses
    ...
    def loss_tokens(self, outputs, targets_all,outputs_ema, indices, num_boxes):
        ...
        loss_token_ema = F.smooth_l1_loss(outputs['memory'][0],outputs['memory'][0])
        # loss_token_ema = 0
        # print(loss_token_ema)
        ...
        for i in range(len(token_pre)):
            ...
            if token_pre_item_part.shape[1]==0:
                continue
            else:
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == 'labels':
                        ...
                        kwargs = {'log': False,'label_class':label_class}
                    # l_dict = self.get_loss(loss, aux_outputs, targets_all,outputs_ema['aux_outputs'][i], indices, num_boxes, **kwargs)
                    if 'SD' in label_class:
                        l_dict = self.get_loss(loss, aux_outputs, targets_all,outputs_ema['aux_outputs'][i], indices, num_boxes, **kwargs)
                    else:
