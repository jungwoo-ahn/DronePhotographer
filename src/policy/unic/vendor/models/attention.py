...
# VENDORED PATCH (torch 2.x compat): the upstream version guard mis-parses on
# torch>=2 (operator precedence) and imports the long-removed `_LinearWithBias`.
# NonDynamicallyQuantizableLinear is an nn.Linear subclass present in modern torch.
try:
    from torch.nn.modules.linear import NonDynamicallyQuantizableLinear as _LinearWithBias
except ImportError:  # very old torch
...
# VENDORED PATCH (torch 2.x compat): same precedence bug; torch._overrides was
# removed. torch.overrides has both symbols in modern torch.
try:
