"""DiffusionPolicyDataset shape/keys test on a synthetic v7 placement."""
...
def test_emits_only_dp_keys(placement):
    ds = DiffusionPolicyDataset([placement], chunk_size=8, stride=2, target_resolution=(32, 48))
    assert len(ds) > 0
    s = ds[0]
...
def test_same_window_count_as_base(placement):
    # must see the identical sample set as the cosmos / vla datasets
    ds = DiffusionPolicyDataset([placement], chunk_size=8, stride=1)
    assert len(ds) == len(ds.base)


def test_default_goal_keys_are_all_eight(placement):
    ds = DiffusionPolicyDataset([placement], chunk_size=8, stride=4)
